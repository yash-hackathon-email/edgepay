...
import { createTransaction, executeUssdTransaction } from '../engine/TransactionEngine';
...
export const SendMoneyScreen: React.FC<{ navigation: any; route: any }> = ({
  ...
  const executePayment = async () => {
    ...
    const txn = createTransaction(numericAmount, receiver, undefined, networkMode);
    addTransaction(txn);
      ...
      <Modal visible={showConfirmModal} transparent animationType="fade">
        <View style={s.modalOverlay}>
          <View style={[s.modalContent, { backgroundColor: colors.surface, borderColor: colors.cardBorder }]}>
            ...
            <Text style={s.modalNote}>Confirm to initialize the secure USSD channel.</Text>
            <View style={s.modalFooter}>
              <TouchableOpacity style={s.modalBtn} onPress={() => setShowConfirmModal(false)}>
                <Text style={{ color: colors.textSecondary }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[s.modalBtn, { backgroundColor: colors.primary + '10' }]} onPress={executePayment}>
                <Text style={{ color: colors.primary, fontWeight: '800' }}>PAY NOW</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  doneBtn: { marginTop: 80, width: '100%', padding: 20, borderRadius: 16, alignItems: 'center' },
});
